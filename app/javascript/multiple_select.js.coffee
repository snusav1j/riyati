    ...
    $.fn.setMultipleSelect = ->
      ...
      options.each ->
        ...
        if $(this).is(':selected')
          ...
          # if show_items_block
          #   multi_select_item_block = $('<div />').addClass('multi-select-item').attr('data-value', $(this).val()).html(text)
          #   input_for_multi_select_block = "<input name='parfum_recipe[#{select_name}][#{$(this).val()}]' type='' value='' autocomplete='off'>"
